...
SELECT `name`,
	   ...
	   case when `monthlymaintenance`>100 then 'expensive'
		else 'cheap' end as `monthly_cost`
FROM `Facilities` 
...
SELECT Facilities.name as `Facility Name`,
	   CASE when Bookings.memid = 0 then Members.firstname else CONCAT (Members.firstname, ' ', Members.surname) end as `Full Name`,
	 CASE when Bookings.memid = 0 then Facilities.guestcost *Bookings.slots
			else Facilities.membercost*Bookings.slots end as `Cost`
FROM `Bookings` 
join Facilities on Facilities.facid = Bookings.facid
join Members on Members.memid = Bookings.memid
WHERE `starttime` > '2012-09-13 23:59:59' 
and   `starttime` < '2012-09-15 00:00:00'
having `COST` >30
order by `cost` desc

/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT sub1.`Facility Name`,
	   sub1.`Full Name`,
	   sub1.`Cost`
From( Select Members.firstname as `Full Name`,
    ...
    AND `starttime` > '2012-09-13 23:59:59' 
	AND   `starttime` < '2012-09-15 00:00:00') sub1
UNION
SELECT sub2.`Facility Name`,
	   sub2.`Full Name`,
	   sub2.`Cost`
From( Select CONCAT (Members.firstname, ' ', Members.surname)  as `Full Name`,
    		Facilities.name as `Facility Name`,
    ...
    Where Bookings.memid!=0
    AND `starttime` > '2012-09-13 23:59:59' 
	AND   `starttime` < '2012-09-15 00:00:00') sub2
...
SELECT Facilities.name as `Facility Name`,
	   SUM(CASE when Bookings.memid = 0 then Facilities.guestcost *Bookings.slots
			else Facilities.membercost*Bookings.slots end) as `Total Revenue`
FROM `Bookings` 
join Facilities on Facilities.facid = Bookings.facid
join Members on Members.memid = Bookings.memid
